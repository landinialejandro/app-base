...
// FILE: app/Http/Requests/UpdateShopItemRequest.php | V2
...
class UpdateShopItemRequest extends FormRequest
{
    ...
    protected function normalizedData(array $data): array
    {
        ...
        $item = $this->route('item');
        $meta = $item instanceof ShopItem && is_array($item->meta)
            ? $item->meta
            : [];

        $meta['commercial'] = $this->commercialData($data, $meta['commercial'] ?? []);
        $data['meta'] = $meta;
    ...
    protected function commercialData(array $data, array $currentCommercial): array
    {
        $defaults = [
            'allow_cart' => true,
            'allow_direct_purchase' => false,
            'stock_policy_mode' => 'inherit',
            'shop_stock_limit' => null,
            'stock_target_quantity' => null,
            'stock_protected_quantity' => null,
            'allow_target_margin' => false,
            'max_quantity_per_checkout' => null,
        ];

        $commercial = $this->has('commercial')
            ? array_replace($defaults, $data['commercial'] ?? [])
            : array_replace($defaults, $currentCommercial);

        return [
            'allow_cart' => $this->has('commercial')
                ? $this->boolean('commercial.allow_cart')
                : (bool) $commercial['allow_cart'],
            'allow_direct_purchase' => $this->has('commercial')
                ? $this->boolean('commercial.allow_direct_purchase')
                : (bool) $commercial['allow_direct_purchase'],
            'stock_policy_mode' => $commercial['stock_policy_mode'],
            'shop_stock_limit' => $this->nullableFloat($commercial['shop_stock_limit']),
            'stock_target_quantity' => $this->nullableFloat($commercial['stock_target_quantity']),
            'stock_protected_quantity' => $this->nullableFloat($commercial['stock_protected_quantity']),
            'allow_target_margin' => $this->has('commercial')
                ? $this->boolean('commercial.allow_target_margin')
                : (bool) $commercial['allow_target_margin'],
            'max_quantity_per_checkout' => $this->nullableInt($commercial['max_quantity_per_checkout']),
        ];
    }
