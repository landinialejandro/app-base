...
class TaskController extends Controller
{
    ...
    {
        $tenant = app('tenant');
        $tasks = Task::query()->with(['project', 'party', 'assignedUser'])->orderByRaw(" CASE WHEN status = ? THEN 1 WHEN status = ? THEN 2 WHEN status = ? THEN 3 WHEN status = ? THEN 4 ELSE 5 END ", [TaskCatalog::STATUS_PENDING, TaskCatalog::STATUS_IN_PROGRESS, TaskCatalog::STATUS_DONE, TaskCatalog::STATUS_CANCELLED,])->orderByRaw(" CASE WHEN due_date IS NULL THEN 1 ELSE 0 END ")->orderBy('due_date')->orderBy('name')->get();
    ...
    public function create(Request $request)
    {
        ...
        $users = User::query()->whereHas('memberships', function ($query) use ($tenant) {
            $query->where('tenant_id', $tenant->id); })->orderBy('name')->get();
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate(['project_id' => ['nullable', 'integer', Rule::exists('projects', 'id')->where(function ($query) use ($tenant) {
            $query->where('tenant_id', $tenant->id)->whereNull('deleted_at'); }),], 'party_id' => ['nullable', 'integer', Rule::exists('parties', 'id')->where(function ($query) use ($tenant) {
                $query->where('tenant_id', $tenant->id)->whereNull('deleted_at'); }),], 'assigned_user_id' => ['nullable', 'integer', Rule::exists('users', 'id'),], 'name' => ['required', 'string', 'max:255'], 'description' => ['nullable', 'string'], 'status' => ['required', 'string', Rule::in(TaskCatalog::statuses()),], 'due_date' => ['nullable', 'date'],]);
        if (!empty($data['assigned_user_id'])) {
            $userBelongsToTenant = User::query()->whereKey($data['assigned_user_id'])->whereHas('memberships', function ($query) use ($tenant) {
                $query->where('tenant_id', $tenant->id); })->exists();
